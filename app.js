...
function agregarAmigo() {
    ...
    if (nombre === "" || !/^[A-Za-zÁáÉéÍíÓóÚúÜü\s]+$/.test(nombre)) {
        alert("Ingrese un nombre válido (solo letras y espacios)."); // Mostrar alerta si el campo no es válido
        return; // Salir de la función
    }

    // Validar que el nombre no esté repetido
    if (amigos.includes(nombre)) {
        alert("El nombre ya está en la lista. No se permiten nombres repetidos."); // Mostrar alerta si el nombre ya existe
        return; // Salir de la función
    }
